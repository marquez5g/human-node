 ...
 */
 ...
 const Human = require('@vladmandic/human').default; // use this when using human in dev mode
 ...
 async function detect(f, file, employees) {
   const input = path.join(f, file);
   ...
   if (result && result.face && result.face.length > 0) {
     for (let i = 0; i < result.face.length; i++) {
       ...
       employees.map(employee => {
         let imageName = employee.photo_url.split("/")[4];
         if (imageName === file) {
           employee.descriptors = face.embedding;
         }
       })
 ...
 async function readEmployees() {
   let employees;
   try {
     const rawdata = fs.readFileSync(process.cwd() + '\\employees.json');
     employees = JSON.parse(rawdata);
   } catch (error) {
     console.log("Error reading employees data from disk", error);
   }
   return employees;
 }
     ...
     if (fs.existsSync(f)) {
       ...
       if (stat.isDirectory()) {
         ...
         for (const file of dir) {
           await detect(f, file, employees);
         }
         // write result to file
         log.data(employees);
         fs.writeFileSync('result.json', JSON.stringify(employees, null, 2));
       } else {
         await detect(f);
       }
     } else {
